...
class _WithdrawScreenState extends State<WithdrawScreen> {
              ...
              validator: (value) {
                ...
                if (viewModel.savingsAccount != null) {
                  final availableBalance =
                      viewModel.savingsAccount!.availableBalance;
                  final amount = double.parse(value);
                  if (amount > availableBalance) {
                    return 'Amount exceeds available balance (${currencyFormat.format(availableBalance)})';
                  }
                }
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Withdrawals may be subject to processing fees depending on your payment method. Please ensure sufficient funds are available.',
                          style: TextStyle(
                            fontSize: 12,
                            color: Colors.amber[900],
                          ),
  ...
  Future<void> _submitWithdrawal(SavingsAccountViewModel viewModel) async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final success = await viewModel.makeWithdrawal(
        amount: amount,
      ...
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Successfully withdrew ${currencyFormat.format(amount)}'),
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                  'Are you sure you want to withdraw ${currencyFormat.format(amount)}?'),
              const SizedBox(height: 12),
              Text(
